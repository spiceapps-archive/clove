...
class ScreenController extends CloveController 
{
	...
	function index() 
	{
		...
		return $this->success("Success","screens");
	}
	...
	function insert()
	{
		...
		return $this->success("Successfully created screen.","screens");
	}
	...
	function removeScreen()
	{
		...
		return $this->notFound("Scene doesn't exist");
...
class ProCrypt {
    ...
    protected function SetMode($mode) {
        ProCrypt::IsSupportedMode($mode) && $this->mode = (int)$mode;
    }
    ...
    protected function SetHash($hash) {
        ProCrypt::IsSupportedHash($hash) && $this->hash = (int)$hash;
    }
    ...
    public static function IsSupportedMode($mode) { 
        return in_array($mode, ProCrypt::SupportedModes());
    }
    ...
    public static function IsSupportedHash($hash) { 
        return in_array($hash, ProCrypt::SupportedHashes());
    }
...
class SceneController extends CloveController 
{
	...
	function index() 
	{
		...
		if($username)
		{
			try
			{
				...
				if(!$user->exists())
				{
					return $this->notFound("User \"$username\" doesn't exist.");
				}
				$user = $user->select()->first();
			}catch(Exception $e)
			{
		...
		return $this->success("Success","scenes");
	}
	...
	function insert()
	{
		...
		if($sc->exists())
		{
			return $this->alreadyExists("You already have a scene with that name.");
		}
		...
		return $this->success("Successfully created Scene.");
	}
	...
	function remove()
	{
		...
		return $this->notFound("No such scene exists.");
	}
...
class SyncController extends CloveController 
{
	...
	function getSync()
	{
		...
		if(!$lastSync)
		{
			return $this->error("No Specified sync date.");
		}
		...
		{
			return $this->notFound("No new sync found.");
		}
		...
		return $this->success("Success","syncs"); //return sync url
	}
	...
	function newSync()
	{
		...
		if(!isset($_FILES['sync']) && !$this->request->data('sync'))
			return $this->notFound("No sync being uploaded.");
		...
		if(!$scene->exists())
		{
			return $this->notFound("Scene not found.");
		}
		...
		if($result)
		{
			return $this->success("Added sync");
		}
		else
		{
			$this->sync->delete();
			return $this->success("Unable to upload sync.");
		}
	...
	private function setSyncPostInfo(&$e,&$sync = null,&$screen = null, &$scene = null)
	{
		...
		try
		{
			$sync->scene    = $scene = $scene  ? $scene  : InputSafety::cleanse($this->request->data('scene'),'integer');
			$sync->screen   = $screen = $screen ? $screen : InputSafety::cleanse($this->request->data('screen'),'integer');
...
class UserController extends AuthController {
	...
	function newForm() {
		if($this->current_user instanceOf User) return $this->redirect('/');
	...
	function insert() {
		...
		$username  = InputSafety::cleanse($this->request->data('username'),'string','invalid uername.');
		$full_name = InputSafety::cleanse($this->request->data('full_name'),'mysqlSafeChars','invalid full name.');
		...
		{
			$this->_form->to(Methods::POST, $this->urlTo('insert'));
			return $this->finished($e->getMessage(),1,"editForm");
		}*/
