...
{	
	...
	public function setPlugin($id = null)
	{
		...
		if(count($errors))
		{
			return $this->ok_error(implode("<br />",$errors),"updateInfo");
		}
	...
	public function addPluginUpdate($id='')
	{
		...
		$update_info = InputSafety::cleanse($this->request->data('update_info'),'*','Invalid update description.');
		
		
		
		if($_FILES["pluginFile"]["error"])
			return $this->ok_error("Please provide a Flex Archive file.","updateVersion");
		...
		if($cv->exists())
		{
			return $this->ok_error("That version already exists.","updateVersion");
		}
	...
	private function compileAS3Archive($file,$version)
	{
		...
		//add the library files so the any SpiceKit libraries aren't included in the command line
		...
		{
			return $this->ok_error($compiler->problemsToString()."<BR /><BR />Shell: <BR />$command","updateVersion");
		}
	...
	{
		if(!($this->current_user instanceOf User)) return $this->ok_error("You must <a href=\"".$this->urlTo("UserController::login")."\">log in</a> to see this page.");
	...
	private function verifyPluginOwner($id)
	{
		...
		{
			return $this->ok_error("This plugin doesn't belong to you.");
		}
