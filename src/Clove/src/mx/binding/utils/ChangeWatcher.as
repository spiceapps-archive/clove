...
package mx.binding.utils
{
	...
	import com.spice.recycle.events.DisposeEvent;
	...
	public class ChangeWatcher
	{
			...
			if (chain.length > 0)
			{
				var w:ChangeWatcher =
					new ChangeWatcher(chain[0], handler, commitOnly,
						watch(null, chain.slice(1), handler, commitOnly),useWeakReference);
				w.reset(host);
				return w;
			}
			else
									  ...
									  useWeakReference:Boolean = false)
		{
			...
			this._useWeakReference = false;useWeakReference;
		...
		public function reset(newHost:Object):void
		{
			...
			if (host != null)
			{
				...
				for (p in events)
				{
					host.addEventListener(p, wrapHandler, false, EventPriority.BINDING, this._useWeakReference);
				}
			}
		...
		/*
		  added by craig condon, on dispose of the host, dispose this current change watcher 
		  @return 
		  
		 */	
		
		private function onNewHostDispose(event:DisposeEvent):void
		{
