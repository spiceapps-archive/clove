			...
			switch (resultType) {
				...
				case RESULT_TYPE_HEADER:
					...
					if (headerRealm.length > 0)
						data.push("realm=\"" + headerRealm + "\"");
					for (var param:Object in _requestParams) {
						// if this is an oauth param, include it
						if (param.toString().indexOf("oauth") == 0) {
							data.push(param + "=\"" + URLEncoding.encode(_requestParams[param]) + "\"");
						}
					}

					var ret3:URLRequestHeader = new URLRequestHeader("Authorization","OAuth "+data.join(","));
					return ret3;
				break;
			}
		}
