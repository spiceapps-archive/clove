	...
	public class OAuthRequest
	{
		...
		*/
		public function buildRequest(signatureMethod:IOAuthSignatureMethod,resultType:String=RESULT_TYPE_URL_STRING,headerRealm:String=""):* {
			...
			_requestParams["oauth_nonce"] = 'blah';//uuid;//Base64.encode();
			_requestParams["oauth_timestamp"] = 'blah';//String(curDate.time).substring(0, 10);
			...
			switch (resultType) {
				...
				case RESULT_TYPE_HEADER:
					...
					for (var param:Object in _requestParams) {
						...
						{
							vars[param] = URLEncoding.encode(_requestParams[param]);
				   ...
				   return new URLRequestHeader("Authorization",data);
					...
					var request:URLRequest = new URLRequest(this._requestURL);
					request.data = vars;
					
					request.method = URLRequestMethod.POST;//this._httpMethod;
//					request.contentType = "application/x-www-form-urlencoded";
					request.requestHeaders = [new URLRequestHeader("Authorization",data)];
					return request;
				break;
			}
		}
