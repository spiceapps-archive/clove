<?php
	...
	class MysqlConnect
	{
		...
		function execute($query,$values = null)
		{
			...
			if(!($new_query = $this->getQuery($query,$values)))
				return false;
		...
		function getQuery($query,$values = null)
		{
			...
			if($values)
			{
				...
				{					
					...
					//if the value is an array, then it contains an error message to return
					if(is_array($v))
					{
						$value = $v[0];
						$message = $v[1];
